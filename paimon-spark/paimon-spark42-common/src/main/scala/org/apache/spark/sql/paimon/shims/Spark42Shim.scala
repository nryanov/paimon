...
class Spark42Shim extends SparkShim {
  ...
  override def isBuiltinFunction(spark: SparkSession, name: String): Boolean =
    spark.sessionState.catalogManager.v1SessionCatalog.isBuiltinFunction(name)
  ...
  override def describeTableRows(
    ...
    DescribeTableExec(output, catalogName, identifier, table, isExtended).executeCollect()
